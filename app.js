...
function renderResources() {
  ...
  _resources.forEach(resource => {
    ...
    /* ── DOM seguro: nunca innerHTML con datos externos ─────────────── */
    const card  = document.createElement("div");
    card.className = "resource-card";

    const title = document.createElement("h3");
    title.textContent = rawNombre;   // textContent escapa automáticamente
    ...
    if (Security.isSafeUrl(rawLink)) {
      ...
      link.target      = "_blank";
      link.rel         = "noopener noreferrer";   // previene tabnapping
